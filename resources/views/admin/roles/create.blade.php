        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-2xl text-secondary leading-tight">
                    {{ trans('messages.Create New Role') }}
                </h2>
                <p class="text-sm text-gray-500 mt-1">
                    {{ trans('messages.إضافة دور جديد') }}
                </p>
            </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required
                                   ...
                                   placeholder="{{ trans('messages.e.g., Administrator') }}">
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3"
                                      ...
                                      placeholder="{{ trans('messages.Role description...') }}">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                                            ...
                                            @foreach($groupPermissions as $permission)
                                                <label class="flex items-start">
                                                    ...
                                                    <div class="ml-2 flex-1">
                                                        ...
                                                        @endif
                                                        <div class="text-xs text-gray-400 mt-0.5 font-mono">{{ $permission->slug }}</div>
                                                    </div>
                                                </label>
                                            @endforeach
                                        </div>
